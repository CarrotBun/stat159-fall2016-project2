...
Lastly, we were able to use run the regressions on the entire credit data, and arrive at the following coefficients:

```{r results= 'asis', echo =FALSE}
ridge_lasso <- data.frame(sapply(c(ridge_final, lasso_final),as.matrix))

rownames(ridge_lasso) =  names(coef(OLS_final))
colnames(ridge_lasso) = c("Ridge", "Lasso")

t(ridge_lasso)
#tb_RL <- xtable(ridge_lasso)
#print(tb_RL, comment = FALSE)
```


#### Principal Component Regression and Partial Least Sqauares Regression
